...
extension String {
    //正規表現の検索をします
    public func pregMatch(pattern: String, options: NSRegularExpression.Options = []) -> Bool {
    ...
    //正規表現の検索結果を利用できます
    public func pregMatch(pattern: String, options: NSRegularExpression.Options = []) -> [String] {
        guard let regex = try? NSRegularExpression(pattern: pattern, options: options) else {
            return []
        }
        var matches: [String] = []
