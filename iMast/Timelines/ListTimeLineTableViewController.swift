...
class ListTimeLineTableViewController: TimeLineTableViewController {
    ...
    override func refreshTimeline() {
        MastodonUserToken.getLatestUsed()?.get("timelines/list/\(listId)?limit=40&since_id="+(self.posts.count >= 1 ? self.posts[0].id.string : "")).then { (res: JSON) in
    ...
    override func readMoreTimeline() {
        MastodonUserToken.getLatestUsed()?.get("timelines/list/\(listId)?limit=40&max_id="+self.posts[self.posts.count-1].id.string).then { (res: JSON) in
            if (res.array != nil) {
                print(res.array)
